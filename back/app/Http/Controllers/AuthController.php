...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        $data = $request->only('name', 'email', 'password', 'rolesId');
        //Realizamos las validaciones
        $validator = Validator::make($data, [
            'name' => 'required|string',
            'email' => 'required|email|unique:users',
            'password' => 'required|string|min:6|max:50',
            'rolesId' => 'required|string',
        ]);
    ...
    public function authenticate(Request $request)
    {
        ...
        //Devolvemos el token
        $usuarios_hoteles = json_decode(DB::table('usuarios_hoteles')->where('userId', Auth::user()->id)->get());
        $user = Auth::user();
        //
        //Arreglamos la imagen de Avatar
        //
        if (!file_exists($user->avatar)) 
            $user->avatar = "imagenes/sinimagen.jpg";

        return response()->json([
            'token' => $token,
            'user' => Auth::user(),
            'usuarios_hoteles' => $usuarios_hoteles
        ]);
    }
